...
try {
  ...
  assert.equal(await b.evaluate(`controller(${JSON.stringify(selector)},'当前场景').querySelector('select').value`), '场景2·标本纵深');
  assert.equal(await b.evaluate(`getComputedStyle(folder(${JSON.stringify(pollenFolder)})).display`), 'none');

  await b.set(selector, '当前场景', '场景3·花粉星云');
  await b.until(`__observed.some(object => object.isScene && object.name === '场景3·花粉星云')`);
  await b.evaluate(`
    window.scene2 = __observed.findLast(object => object.isScene && object.name === '场景3·花粉星云');
    window.pollenRoot = scene2.getObjectByName('场景3·幽兰花粉星云');
  ...
  report.isolation = await b.evaluate(`({
    ...
    pollenInScene1: !!scene.getObjectByName('场景3·幽兰花粉星云'),
  ...
  report.layers = await b.evaluate(`({
    ...
    scene2FolderVisible: getComputedStyle(folder(['场景3·花粉星云'])).display !== 'none',
  })`);
  ...
  await b.set(selector, '当前场景', '场景2·标本纵深');
  await b.delay(250);
  ...
  await b.evaluate(`folder(['场景3·花粉星云']).classList.remove('closed');controller(['场景3·花粉星云'],'能量核心大小').scrollIntoView({block:'center'});`);
  await b.delay(150);
  assert.ok(await b.evaluate(`(()=>{const r=controller(['场景3·花粉星云'],'能量核心大小').getBoundingClientRect();return r.top>=0&&r.bottom<=844;})()`));
