...
const b=await browserHarness(output), panel=['场景12·字符物理实验'], select=['场景选择'];
const report={};
...
try {
  ...
  await b.evaluate(`window.cs=__observed.findLast(o=>o.name==='场景12·字符物理实验');void 0;`);
  assert.equal((await read()).ready,false,'engine stays lazy outside scene12');
  ...
  assert.equal(report.options.at(-1),'场景12·字符物理实验');
  assert.ok(['场景9·纵深花廊','场景10·狮城手记','场景11·纸间来信'].every(name=>report.options.includes(name)),'existing scenes coexist with explicitly numbered scene12');
  await b.set(select,'当前场景','场景12·字符物理实验');
  ...
  await b.evaluate(`folder(['场景12·字符物理实验']).classList.remove('closed');void 0;`);
  ...
  await b.set(select,'当前场景','场景8·斑驳光影');await b.delay(250);
  await b.set(select,'当前场景','场景2·标本纵深');await b.delay(250);
  ...
  for(let i=0;i<3;i++) {
    await b.set(select,'当前场景','场景2·标本纵深');await b.set(select,'当前场景','场景12·字符物理实验');
  ...
  // Keyboard transport applies only to scene12 and never hijacks text entry.
  ...
  await b.send('Page.navigate',{url:new URL('?scene=12',base).href});
  ...
  await capture('05-direct-scene12.png');
  report.directScene12=true;report.errors=b.errors;
