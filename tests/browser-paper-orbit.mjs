...
const select = ['场景选择'], panel = ['场景6·纸飞机环游'];
try {
  ...
  await b.set(select, '当前场景', '场景6·纸飞机环游');
  ...
  for (let i = 0; i < 4; i++) {
    ...
    await b.set(select, '当前场景', '场景6·纸飞机环游'); await b.delay(60);
  }
  ...
  await b.set(select, '当前场景', '场景5·无限花开'); await b.delay(150);
  await b.set(select, '当前场景', '场景6·纸飞机环游'); await b.delay(100);
  ...
  await b.click(panel, '重置纸飞机环游');
  await b.evaluate(`folder(['场景6·纸飞机环游']).querySelector(':scope > .title').click()`);
  await b.delay(350);
